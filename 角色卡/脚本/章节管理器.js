...
function getChapterData() {
    ...
    if (value === null) {
        value = JSON.stringify({ chapter_iter: 0, chapter_name: chapter_list[0] });
        window.localStorage.setItem(chat_id, value);
    }
...
function setChapterData(data) {
    const chat_id = SillyTavern.getCurrentChatId();
    window.localStorage.setItem(chat_id, JSON.stringify(data));
}

// 新聊天时初始化章节数据
eventOn(iframe_events.MESSAGE_IFRAME_RENDER_STARTED, async () => {
    const data = getChapterData();
    await detector.reset_chapter();
    await detector.onChapterChange(data.chapter_name, true);
    toastr.info('章节数据初始化成功');
});

// 变量更新后检查是否需要推进章节
// 从 MVU 变量中读取 is_changed_chapter，如果为 'GO' 则推进到下一章节
eventOn(Mvu.events.VARIABLE_UPDATE_ENDED, async (variables) => {
    if (_.get(variables, 'stat_data.chapter.is_changed_chapter') !== 'GO') return;

    try {
        const data = getChapterData();
        const last_iter = data.chapter_iter;

        if (last_iter >= chapter_list.length - 1) {
            toastr.error("已经是最后一章了");
            _.set(variables, 'stat_data.chapter.is_changed_chapter', 'NO');
            await Mvu.replaceMvuData(variables, { type: 'chat' });
            return;
        }

        const new_iter = last_iter + 1;
        data.chapter_iter = new_iter;
        data.chapter_name = chapter_list[new_iter];
        setChapterData(data);
        ...
        toastr.info(`章节推进成功: ${chapter_list[new_iter]}`);
    } catch (e) {
        console.error('章节变更器出错:', e);
    }
});